...
/datum/wires/jukebox/UpdatePulsed(var/index)
	...
	switch(index)
		if(WIRE_POWER)
			holder.visible_message("<span class='notice'>[icon2html(holder, viewers(holder))] The power light flickers.</span>")
			A.shock(usr, 90)
		if(WIRE_HACK)
			holder.visible_message("<span class='notice'>[icon2html(holder, viewers(holder))] The parental guidance light flickers.</span>")
		if(WIRE_REVERSE)
			holder.visible_message("<span class='notice'>[icon2html(holder, viewers(holder))] The data light blinks ominously.</span>")
		if(WIRE_SPEEDUP)
			holder.visible_message("<span class='notice'>[icon2html(holder, viewers(holder))] The speakers squeaks.</span>")
		if(WIRE_SPEEDDOWN)
			holder.visible_message("<span class='notice'>[icon2html(holder, viewers(holder))] The speakers rumble.</span>")
		if(WIRE_START)
			A.StartPlaying()
		if(WIRE_STOP)
			A.StopPlaying()
