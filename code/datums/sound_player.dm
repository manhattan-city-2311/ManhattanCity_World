...
/decl/sound_player/proc/PlayLoopingSound(atom/source, sound_id, sound, volume, range, falloff, prefer_mute)
...
/decl/sound_player/proc/PrivGetChannel(sound_id)
...
// environment = 0 ensures a 3D effect even if x/y offset happens to be 0 the first time it's played
/datum/sound_token/New(atom/source, sound_id, sound, volume, channel, range = 4, falloff = 1, prefer_mute = FALSE)
	..()
	listeners = list()
...
/datum/sound_token/proc/PrivAddListener(atom/listener)
	if(istype(listener, /mob/observer/virtual))
	...
	if(ismob(listener))
		var/mob/M = listener
		S.environment = M.get_sound_env(1)
	else
		S.environment = get_area(listener).sound_env
	S.falloff = falloff
...
/datum/sound_token/proc/PrivUpdateListener(listener, update_sound)
	var/sound/S = listeners[listener]
	S.volume = volume
...
/obj/sound_test
	var/sound = 'sound/vehicles/ambulance_siren.ogg'

/obj/sound_test/New()
	..()
	global.sound_player.PlayLoopingSound(src, /obj/sound_test, sound, 50, 3)
