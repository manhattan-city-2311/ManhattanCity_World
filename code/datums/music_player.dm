...
/datum/music_player/Topic(herf, href_list)
	...
	if(instrument.Adjacent(usr) && isliving(usr) && !issilicon(usr))
		if(href_list["newsong"])
			...
			var/newline = sanitize(input("Enter new line: ") as text|null, MAX_LINE_SIZE)
			...
			var/content = sanitize(input("Enter your line: ", "Change line [line_num]", song_lines[line_num]) as text|null, MAX_LINE_SIZE)
			...
			var/song_text = sanitize(input("Please, paste the entire song formatted: ") as message|null, MAX_SONG_SIZE, extra = FALSE)
