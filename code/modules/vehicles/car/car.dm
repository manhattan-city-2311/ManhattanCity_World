...
/obj/vehicle/car/examine(mob/user)
	..()
	if(has_license)
		to_chat(user, "The license plate reads <b>[license_plate_no]</b> in bold black letters.")
	to_chat(user, "The power light is [on ? "on" : "off"].\nThere are[key ? "" : " no"] keys in the ignition.")
	to_chat(user, "The charge meter reads [cell? round(cell.percent(), 0.01) : 0]%")
	to_chat(user, "Car integrity is [health]/[maxhealth].")
...
/obj/vehicle/car/attackby(obj/item/weapon/pen/crayon/spraycan/W as obj, mob/user as mob)
	if(istype(W, /obj/item/weapon/pen/crayon/spraycan))
		if(W.capped)
			to_chat(user, "The spraycan is still capped! Uncap it first.")
		else
			var/car_color = W.colour
			to_chat(user, "You start painting the [src].")
