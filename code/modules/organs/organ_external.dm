...
/obj/item/organ/external
	...
	var/gendered_icon = FALSE              // Whether or not the icon state appends a gender.
...
/obj/item/organ/external/proc/relocate()
	...
	if(owner)
		...
		for(var/obj/item/organ/external/limb in owner.organs_by_name)
...
/obj/item/organ/external/remove_rejuv()
	if(owner)
		...
		owner.organs_by_name[organ_tag] = null
		owner.organs_by_name -= organ_tag
		while(null in owner.organs_by_name) owner.organs_by_name -= null
...
/obj/item/organ/external/proc/createwound(type = CUT, damage)
	...
	if((damage > 15) && (type != BURN) && (local_damage > 30) && prob(damage) && (robotic < ORGAN_ROBOT) && !(species.flags & NO_BLOOD))
		var/datum/wound/internal_bleeding/I = new (min(damage - 15, 5))
		wounds += I
...
//Burn damage can cause fluid loss due to blistering and cook-off
	if((damage > 5 || damage + burn_dam >= 15) && type == BURN && (robotic < ORGAN_ROBOT) && !(species.flags & NO_BLOOD))
		var/fluid_loss = 0.4 * (damage/(owner.getMaxHealth() - config.health_threshold_dead)) * owner.species.blood_volume
...
/obj/item/organ/external/proc/droplimb(clean, disintegrate = DROPLIMB_EDGE, ignore_children = null)
	...
	playsound(owner, 'sound/effects/sound_effects_gore_severed.ogg', 50, 1)
		...
		if(!keep_organs)
			for(var/obj/item/organ/thing in internal_organs)
				if(istype(thing))
					...
					owner.internal_organs_by_name.Remove(thing)
					qdel(thing)
