...
/obj/machinery/portable_atmospherics/hydroponics/attack_generic(var/mob/user)
	...
	if(istype(user,/mob/living/carbon/alien/diona))
		...
		if(weedlevel > 0)
			...
			weedlevel = 0
			nymph.visible_message(SPAN_INFO("<b>[nymph]</b> begins rooting through [src], ripping out weeds and eating them noisily."), SPAN_INFO("You begin rooting through [src], ripping out weeds and eating them noisily."))
		else if(nymph.nutrition > 100 && nutrilevel < 10)
			nymph.nutrition -= ((10-nutrilevel)*5)
			nutrilevel = 10
			nymph.visible_message(SPAN_INFO("<b>[nymph]</b> secretes a trickle of green liquid, refilling [src]."), SPAN_INFO("You secrete a trickle of green liquid, refilling [src]."))
		else
			nymph.visible_message(SPAN_INFO("<b>[nymph]</b> rolls around in [src] for a bit."), SPAN_INFO("You roll around in [src] for a bit."))
		return
