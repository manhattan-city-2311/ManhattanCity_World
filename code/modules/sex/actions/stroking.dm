...
/datum/erp_action/hips_stroke/get_messages(mob/living/carbon/human/user1, mob/living/carbon/human/user2)
	if(user2.get_covered_body_parts() & LOWER_TORSO)
		return list(
			"@1 касается талии @2, проводит рукой вниз, и вот [pronounce_helper(user2, "его", "её")] рука уже покоится на бедре @2.",
			"@1 плотно сжимает бедро @2.",
			"@1 кладет свои ладони на бедра @2 и водит по ним вверх-вниз."
		)
	else
		return list(
			"@1 кладет свою руку на бедро @2, медленно оглаживая кожу.",
