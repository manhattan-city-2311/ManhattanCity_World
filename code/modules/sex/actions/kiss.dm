...
/datum/erp_action/kiss_cheek/rough/get_messages(user1, user2)
	return list(
		"@1 резко наклоняется вперёд, вторгаясь в личное пространство @2 и клюёт [pronounce_helper(user2, "его", "её")] в щеку, тут же отстранившись."
	)
...
/datum/erp_action/kiss/get_messages(user1, user2)
	var/list/height_dependent_messages = list(
		list("@1 движется вперёд, и подтягивается, с усилием оказываясь напротив @2 и тут же запечатляет поцелуй на [pronounce_helper(user1, "его", "её")] губах, проникая языком внутрь, касаясь зубов @2."),
		...
		list("@1 наклоняется, даже сгибается к @2 обхватывая [pronounce_helper(user2, "его", "её")] за плечи, касается своими губами губ @2, перебирает пальцами по плечам @2, углубляет поцелуй, проводит своим языком по зубам @2 и только теперь отстраняется, довольно тяжело дыша.")
	)
