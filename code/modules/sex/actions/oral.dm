...
/datum/erp_action/blowjob/get_messages(mob/living/carbon/human/user1, mob/living/carbon/human/user2)
	...
	switch(get_stage(user1))
		...
		if(2)
			if(poses ~= list(POS_KNEELING, POS_STANDING))
				return "@1 дёргает головой, насаживаясь ртом на ствол, не до основания но близко к тому, задыхается, [pronounce_helper(user2)] лицо покраснело."
			else if(poses ~= list(POS_KNEELING, POS_SITTING))
				return "@1 активно засасывает чувствительную плоть, играя языком внутри своего рта. Глаза @1 слезятся от усердия."
			else
				return "@1 довольно быстро двигает годовой, и плотно обхватывает ствол губами, практически дроча член @2 своим ртом."
