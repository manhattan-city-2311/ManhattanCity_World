...
datum/admins/proc/notes_show(var/ckey)
	to_target(usr, browse("<head><title>Player Notes</title></head><body>[notes_gethtml(ckey)]</body>","window=player_notes;size=700x400"))
...
/proc/notes_add(var/key, var/note, var/mob/user)
	...
	message_admins(SPAN_INFO("[key_name_admin(user)] has edited [key]'s notes."))
...
/proc/notes_del(var/key, var/index)
	...
	message_admins(SPAN_INFO("[key_name_admin(usr)] deleted one of [key]'s notes."))
