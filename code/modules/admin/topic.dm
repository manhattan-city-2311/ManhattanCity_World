/datum/admins/Topic(href, href_list)
	...
	if(href_list["dbsearchckey"] || href_list["dbsearchadmin"])
		...
		switch(href_list["call_shuttle"])
			if("1")
				...
				if (emergency_shuttle.can_call())
					...
					message_admins(SPAN_INFO("[key_name_admin(usr)] called the Emergency Shuttle to the city."), 1)
			...
			if("2")
				...
				if (emergency_shuttle.can_call())
					...
					message_admins(SPAN_INFO("[key_name_admin(usr)] sent the Emergency Shuttle back."), 1)
		...
		if (emergency_shuttle.wait_for_launch)
			...
			message_admins(SPAN_INFO("[key_name_admin(usr)] edited the Emergency Shuttle's launch time to [new_time_left*10]"), 1)
			...
			message_admins(SPAN_INFO("[key_name_admin(usr)] edited the Emergency Shuttle's arrival time to [new_time_left*10]"), 1)
		else
		...
		message_admins(SPAN_INFO("[key_name(usr)] [ticker.delay_end ? "delayed the round end" : "has made the round end normally"]."), 1)
		href_list["secretsadmin"] = "check_antagonist"
		...
		message_admins(SPAN_INFO("[key_name_admin(usr)] has used rudimentary transformation on [key_name_admin(M)]. Transforming to [href_list["simplemake"]]; deletemob=[delmob]"), 1)
	...
	/////////////////////////////////////new ban stuff
		...
		message_admins(SPAN_INFO("[key_name_admin(usr)] edited [banned_key]'s ban. Reason: [reason] Duration: [duration]"), 1)
		Banlist.cd = "/base/[banfolder]"
...
//Regular jobs
	...
	//Antagonist (Orange)
		...
		for(var/antag_type in all_antag_types)
			...
			if(jobban_isbanned(M, "[antag.bantype]") || isbanned_dept)
				jobs += "<td width='20%'><a href='?src=\ref[src];jobban3=[antag.bantype];jobban4=\ref[M]'><font color=red>[replacetext_char("[antag.role_text]", " ", "&nbsp")]</font></a></td>"
			else
				jobs += "<td width='20%'><a href='?src=\ref[src];jobban3=[antag.bantype];jobban4=\ref[M]'>[replacetext_char("[antag.role_text]", " ", "&nbsp")]</a></td>"
	...
	//JOBBAN'S INNARDS
		...
		if(notbannedlist.len) //at least 1 unbanned job exists in joblist so we have stuff to ban.
			switch(alert("Temporary Ban?",,"Yes","No", "Cancel"))
				if("Yes")
					...
					message_admins(SPAN_INFO("[key_name_admin(usr)] banned [key_name_admin(M)] from [msg] for [mins] minutes"), 1)
				...
				if("No")
					...
					if(reason)
						...
						notes_add(M.ckey, "Banned  from [msg] - [reason]", usr)
						message_admins(SPAN_INFO("[key_name_admin(usr)] banned [key_name_admin(M)] from [msg]"), 1)
						to_chat(M, "<font color='red'><BIG><B>You have been jobbanned by [usr.client.ckey] from: [msg].</B></BIG></font>")
						to_chat(M, "<font color='red'><B>The reason is: [reason]</B></font>")
			...
			for(var/job in joblist)
				...
				switch(alert("Job: '[job]' Reason: '[reason]' Un-jobban?","Please Confirm","Yes","No"))
					if("Yes")
						...
						continue
			if(msg)
				message_admins(SPAN_INFO("[key_name_admin(usr)] unbanned [key_name_admin(M)] from [msg]"), 1)
		...
		if (ismob(M))
			...
			if(!reason)
				to_chat(M, "<font color='red'>You have been kicked from the server</font>")
			else
				to_chat(M, "<font color='red'>You have been kicked from the server: [reason]</font>")
			log_admin("[key_name(usr)] booted [key_name(M)].")
			message_admins(SPAN_INFO("[key_name_admin(usr)] booted [key_name_admin(M)]."), 1)
			//M.client = null
			qdel(M.client)
		...
		if(t)
			if((alert("Do you want to unjobban [t]?","Unjobban confirmation", "Yes", "No") == "Yes") && t) //No more misclicks! Unless you do it twice.
				...
				message_admins(SPAN_INFO("[key_name_admin(usr)] removed [t]"), 1)
				jobban_remove(t)
		...
		switch(alert("Temporary Ban?",,"Yes","No", "Cancel"))
			if("Yes")
				...
				message_admins(SPAN_INFO("[usr.client.ckey] has banned [M.ckey].\nReason: [reason]\nThis will be removed in [mins] minutes."))
		...
		message_admins(SPAN_INFO("[key_name_admin(usr)] set the forced secret mode as [secret_force_mode]."), 1)
		Game() // updates the main game menu
		...
		message_admins(SPAN_INFO("[key_name_admin(usr)] attempting to monkeyize [key_name_admin(H)]"), 1)
		H.monkeyize()
		...
		message_admins(SPAN_INFO("[key_name_admin(usr)] attempting to corgize [key_name_admin(H)]"), 1)
		H.corgize()
		...
		message_admins(SPAN_INFO("[key_name_admin(usr)] forced [key_name_admin(M)] to say: [speech]"))
		...
		message_admins(SPAN_INFO("[key_name_admin(usr)] sent [key_name_admin(M)] to the prison station."), 1)
	...
	// call dibs on IC messages (prays, emergency comms, faxes)
		...
		if(isliving(M))
			...
			health_description += "<BR>Oxy: [L.get_deprivation()] - Tox: [L.getToxLoss()] - Fire: [L.getFireLoss()] - Brute: [L.getBruteLoss()] - Clone: [L.getCloneLoss()] - Brain: [L.getBrainLoss()]"
		else
