/datum/database/accounts
	...
	contents = list(
		"system" = list(
			...
			"permissions" = list(PERMISSION_ADMIN)
		)
	)
...
/datum/database/accounts/proc/add_permission(username, permissions, account = "system")
	...
	for(var/P in permissions)
		if(findtext(P, PERMISSION_ADMIN))
...
/datum/database/accounts/proc/access(username, password, dbname)
	...
	if(PERMISSION_ADMIN in entry["permissions"])
		. = list()
		for(var/P in list(PERMISSION_ACCESS_LOGS, PERMISSION_READ, PERMISSION_WRITE))
			. += "[dbname][P]"
		return .
