...
/obj/machinery/modular_computer/console/New()
	...
	if(A && console_department)
		cpu.network_card.identification_string = replacetext_char(replacetext_char(replacetext_char("[A.name] [console_department] Console", " ", "_"), "-", ""), "__", "_") // Replace spaces with "_"
	else if(A)
		cpu.network_card.identification_string = replacetext_char(replacetext_char(replacetext_char("[A.name] Console", " ", "_"), "-", ""), "__", "_")
	else if(console_department)
		cpu.network_card.identification_string = replacetext_char(replacetext_char(replacetext_char("[console_department] Console", " ", "_"), "-", ""), "__", "_")
	else
