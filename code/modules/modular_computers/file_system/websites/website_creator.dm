...
/datum/website/creator/on_access(user)
	...
	new_content = replacetext_char(new_content, "https://", "{LINK EXPUNGED}")
	new_content = replacetext_char(new_content, "www.", "{LINK EXPUNGED}")
	new_content = replacetext_char(new_content, "http://", "{LINK EXPUNGED}")
	new_content = replacetext_char(new_content, "href", "{HREF REMOVED}") //HREF exploits are a pain in the ass.
