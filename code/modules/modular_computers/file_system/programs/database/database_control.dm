...
/datum/nano_module/program/databaseControl/ui_interact(mob/user, ui_key = "main", datum/nanoui/ui, force_open = TRUE, datum/topic_state/state = default_state)
	...
	if(loggedin)
		data["viewLogs"] = (!"[database][PERMISSION_ACCESS_LOGS]" in permissions) ? -1 : viewLogs
		if(viewLogs)
			data["logs"] = DB.logs
		else if(("[database][PERMISSION_READ]" in permissions))
			var/list/L = list()
	...
	if(!ui)
		ui = new(user, src, ui_key, "database_control.tmpl", "MQL database control", 600, 400, state = state)
...
/datum/nano_module/program/databaseControl/Topic(href, href_list)
	...
	if(href_list["view_logs"])
		if(viewLogs != 0 || !permissions || !("[database][PERMISSION_ACCESS_LOGS]" in permissions))
			return
	...
	if(href_list["login"])
		...
		permissions = accountsDB.access(username, password, database)
		if(!permissions)
