...
/obj/machinery/suspension_gen/proc/activate()
	...
	for(var/mob/living/M in T)
		M.weakened += 5
		M.visible_message(SPAN_INFO("\icon[M] [M] begins to float in the air!"),"You feel tingly and light, but it is difficult to move.")

	suspension_field = new(T)
	src.visible_message(SPAN_INFO("\icon[src] [src] activates with a low hum."))
	...
	if(collected)
		suspension_field.icon_state = "energynet"
		suspension_field.overlays += "shield2"
		src.visible_message(SPAN_INFO("\icon[suspension_field] [suspension_field] gently absconds [collected > 1 ? "something" : "several things"]."))
	else
...
/obj/machinery/suspension_gen/proc/deactivate()
	...
	src.visible_message(SPAN_INFO("\icon[src] [src] deactivates with a gentle shudder."))
