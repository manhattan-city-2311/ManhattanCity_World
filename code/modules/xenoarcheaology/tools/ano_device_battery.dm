	...
	if(activated)
		if(inserted_battery && inserted_battery.battery_effect && (inserted_battery.stored_charge > 0) )
			...
			if(inserted_battery.stored_charge <= 0)
				src.loc.visible_message(SPAN_INFO("\icon[src] [src] buzzes."), "<font color='blue'>\icon[src] You hear something buzz.")
				shutdown_emission()
			else if(world.time > time_end)
				src.loc.visible_message(SPAN_INFO("\icon[src] [src] chimes."), "<font color='blue'>\icon[src] You hear something chime.")
				shutdown_emission()
		else
			src.visible_message(SPAN_INFO("\icon[src] [src] buzzes."), "<font color='blue'>\icon[src] You hear something buzz.")
			shutdown_emission()
...
/obj/item/weapon/anodevice/Topic(href, href_list)
	...
	if(href_list["startup"])
		if(inserted_battery && inserted_battery.battery_effect && (inserted_battery.stored_charge > 0) )
			activated = 1
			src.visible_message(SPAN_INFO("\icon[src] [src] whirrs."), "\icon[src]<font color='blue'>You hear something whirr.")
			if(!inserted_battery.battery_effect.activated)
				inserted_battery.battery_effect.ToggleActivate(1)
...
/obj/item/weapon/anodevice/attack(mob/living/M as mob, mob/living/user as mob, def_zone)
	...
	if(activated && inserted_battery.battery_effect.effect == EFFECT_TOUCH && !isnull(inserted_battery))
		...
		inserted_battery.use_power(energy_consumed_on_touch)
		user.visible_message(SPAN_INFO("[user] taps [M] with [src], and it shudders on contact."))
	else
		user.visible_message(SPAN_INFO("[user] taps [M] with [src], but nothing happens."))
