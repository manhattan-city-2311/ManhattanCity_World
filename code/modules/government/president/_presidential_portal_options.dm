...
/datum/persistent_option/proc/sanitize_options()
	...
	if(SSpersistent_options.find_proposed_value_ballot(id))
		...
		on_ballot_pass = replacetext_char(on_ballot_pass, "%NAME", name)
		on_ballot_pass = replacetext_char(on_ballot_pass, "%VALUE", get_formatted_value())
		on_ballot_pass = replacetext_char(on_ballot_pass, "%PROPOSEDVALUE", SSpersistent_options.find_proposed_value_ballot(id))

		on_ballot_fail = replacetext_char(on_ballot_fail, "%NAME", name)
		on_ballot_fail = replacetext_char(on_ballot_fail, "%VALUE", get_formatted_value())
		on_ballot_fail = replacetext_char(on_ballot_fail, "%PROPOSEDVALUE", SSpersistent_options.find_proposed_value_ballot(id))
	return 1
...
/datum/persistent_option/proc/perform_edit_outcome(the_new_value, mob/user)
	if(!make_referendum)
		...
		if(new_referendum && creation_text)
			creation_text = replacetext_char(creation_text, "%NAME", name)
			creation_text = replacetext_char(creation_text, "%VALUE", get_formatted_value())
			creation_text = replacetext_char(creation_text, "%PROPOSEDVALUE", get_proposed_value_formatting())
