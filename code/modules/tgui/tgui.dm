...
/datum/tgui/New(mob/user, datum/src_object, interface, title, ui_x, ui_y)
	var/is_fancy = user?.get_client_preference(/datum/client_preference/tgui_style) == GLOB.PREF_FANCY
	log_tgui(user,
...
/datum/tgui/proc/open()
	...
	if(!window.is_ready())
		window.initialize(
			fancy = user.get_client_preference(/datum/client_preference/tgui_style) == GLOB.PREF_FANCY,
			assets = list(
...
/datum/tgui/proc/_get_payload(custom_data, with_data, with_static_data)
	...
	json_data["config"] = list(
		...
		"status" = status,
		"interface" = interface,
		"theme" = GLOB.PREF_WHITE,
		"window" = list(
			"key" = window_key,
			"size" = window_size,
			"fancy" = user.get_client_preference(/datum/client_preference/tgui_style) == GLOB.PREF_FANCY,
			"locked" = user.get_client_preference(/datum/client_preference/tgui_monitor) == GLOB.PREF_PRIMARY
		),
		"client" = list(
