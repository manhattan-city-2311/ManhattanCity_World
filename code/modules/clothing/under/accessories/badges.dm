...
/obj/item/clothing/accessory/badge
	...
	var/stored_name
	var/badge_string = "Нью-Манхэттен Сити Police Department"
	var/name_reset = ""
...
/obj/item/clothing/accessory/badge/holo/verb/Reset()
	...
	if(istype(id_card) && ((access_security in id_card.access) || emagged))
		if(!stored_name)
			to_chat(usr, "There is no information stored on the badge.")
		else
			to_chat(usr, "You reset the holobadge.")
			stored_name = FALSE
			name = name_reset
	else
		to_chat(usr, "[name] rejects your insufficient access rights.")
	return
...
/obj/item/clothing/accessory/badge/press/verb/Reset()
	...
	if(istype(id_card) && ((access_library in id_card.access) || emagged))
		if(!stored_name)
			...
			to_chat(usr, "You reset the press badge.")
			stored_name = FALSE
			name = name_reset
	else
		to_chat(usr, "[name] rejects your insufficient access rights.")
...
/obj/item/clothing/accessory/badge/press/attack_self(mob/user as mob)
	...
	if(!stored_name)
		to_chat(user, "Waving around a press badge before swiping an ID would be pretty pointless.")
		return
...
/obj/item/clothing/accessory/badge/press/verb/Change_Network()
	...
	if(!isnull(new_news_network_name))
		...
		to_chat(usr, "<span class='notice'>News Network changed to '[new_news_network_name]'.</span>")

/obj/item/clothing/accessory/badge/press/emag_act(remaining_charges, mob/user)
	if (emagged)
		to_chat(user, "<span class='danger'>\The [src] is already cracked.</span>")
		return
	else
		emagged = 1
		to_chat(user, "<span class='danger'>You crack the press badge's security checks.</span>")
		return 1

/obj/item/clothing/accessory/badge/press/attackby(obj/item/O as obj, mob/user as mob)
	if(istype(O, /obj/item/weapon/card/id) || istype(O, /obj/item/device/pda))
