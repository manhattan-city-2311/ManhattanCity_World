	...
	if(ai_mob)
		...
		if(ai_mob.key && ai_mob.client)
			...
			if(istype(ai, /obj/item/device/aicard))
				...
				ai.forceMove(src)
				ai_card = ai
				to_chat(ai_mob, SPAN_INFO("You have been transferred to \the [holder]'s [src]."))
				to_chat(user, SPAN_INFO("You load [ai_mob] into \the [holder]'s [src]."))
...
/obj/item/rig_module/power_sink/proc/drain_complete(var/mob/living/M)
	...
	if(!interfaced_with)
		if(M) to_chat(M, SPAN_INFO("<b>Total power drained:</b> [round(total_power_drained*CELLRATE)] cell units."))
	else
		if(M) to_chat(M, SPAN_INFO("<b>Total power drained from [interfaced_with]:</b> [round(total_power_drained*CELLRATE)] cell units."))
