...
/obj/item/weapon/photo/examine(mob/user)
	if(in_range(user, src))
		...
		user << desc
		if(sensational)
			to_chat(user, "<span class='notice'>The important people in this photo make it highly <b>sensational</b>!</span>")
		if(gruesome)
			to_chat(user, "<span class='notice'>The photo has very <b>gruesome</b> details...</span>")
		if(scandalous)
			to_chat(user, "<span class='notice'><b>S-Scandalous!</b> The media will love this.</span>")
		if(scary)
			to_chat(user, "<span class='notice'>This photo is... <b>scary</b>.</span>")
	else
		to_chat(user, "<span class='notice'>It is too far away.</span>")

/obj/item/weapon/photo/proc/show(mob/user as mob)
	user << browse_rsc(img, "tmp_photo_[id].png")
	user << browse("<meta charset='utf-8'><head><title>[name]</title></head>" \
