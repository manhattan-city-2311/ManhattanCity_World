...
/datum/shuttle/ferry/emergency/move(var/area/origin,var/area/destination)
	...
	if (origin == area_station)	//leaving the station
		...
		if (emergency_shuttle.evac)
			priority_announcement.Announce(replacetext_char(replacetext_char(using_map.emergency_shuttle_leaving_dock, "%dock_name%", "[using_map.dock_name]"),  "%ETA%", "[estimated_time] minute\s"))
		else
			priority_announcement.Announce(replacetext_char(replacetext_char(using_map.shuttle_leaving_dock, "%dock_name%", "[using_map.dock_name]"),  "%ETA%", "[estimated_time] minute\s"))
