...
/obj/item/weapon/gun/proc/Fire(atom/target, mob/living/user, clickparams, pointblank=0, reflex=0)
	...
	if(target.elevation != last_elevation && (istype(target,/obj/manhattan/vehicle) || istype(target,/mob/living)))
	...
	for(var/i in 1 to burst)
		...
		*/
		...
		if(!(user.last_mouse_target && user.last_mouse_target.loc))
			...
			pointblank = 0
	//if(!automatic)
	//	user.setClickCooldown(DEFAULT_QUICK_COOLDOWN) //So you can click pretty quick for a bit before it starts getting innacurate.
	//if(automatic)
