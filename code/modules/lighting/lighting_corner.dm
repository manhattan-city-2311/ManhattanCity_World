...
/datum/lighting_corner/proc/update_overlays()
	...
	var/lum_r = src.lum_r > 0 ? LIGHTING_MULT_FACTOR * sqrt(src.lum_r ** 1.9 + 0.05) : src.lum_r
	var/lum_g = src.lum_g > 0 ? LIGHTING_MULT_FACTOR * sqrt(src.lum_g ** 1.9 + 0.05) : src.lum_g
	var/lum_b = src.lum_b > 0 ? LIGHTING_MULT_FACTOR * sqrt(src.lum_b ** 1.9 + 0.05) : src.lum_b
