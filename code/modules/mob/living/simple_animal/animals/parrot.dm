		...
		if(in_range(src, parrot_interest))
			...
			if(isliving(parrot_interest))
				...
				if(!parrot_perch || parrot_interest.loc != parrot_perch.loc)
					...
					visible_message("[src] grabs the [held_item]!", SPAN_INFO("You grab the [held_item]!"), "You hear the sounds of wings flapping furiously.")
...
/mob/living/simple_animal/parrot/proc/steal_from_mob()
	...
	for(var/mob/living/carbon/C in view(1,src))
		...
		if(stolen_item)
			...
			visible_message("[src] grabs the [held_item] out of [C]'s hand!", SPAN_INFO("You snag the [held_item] out of [C]'s hand!"), "You hear the sounds of wings flapping furiously.")
			return held_item
