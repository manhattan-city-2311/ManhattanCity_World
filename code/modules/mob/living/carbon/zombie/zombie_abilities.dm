/mob/living/carbon/human/proc/revive_undead()
	...
	if(src.stat == DEAD)
		if(!has_brain(C))
			to_chat(C, "<span class='notice'>You have no vessel! The connection between the parasitic entity within you and your brain is severed. You are dead, for real.</span>")
			return
		else
			dead_mob_list -= src
			living_mob_list += src
			...
			if(!isundead(C))
				return 0
			to_chat(C, "<span class='notice'>The parasitic entity inside you begins knitting your decomposing form back together. This will take <b>three and a half minutes</b>.</span>")
			...
			if(do_after(user = C, delay = 2100, needhand = 0, incapacitation_flags = null))
				...
				if(ishuman(C))
					...
					to_chat(C, "<span class='notice'>You rise from the dead.</span>")
					C.update_canmove()
					C.stat = CONSCIOUS
				...
				to_chat(C, "<span class='notice'>Your vessel is interrupted.</span>")

	else
		to_chat(C, "<span class='alium'>You can only use this while dead.</span>")
		return
...
/mob/living/carbon/human/proc/fermented_goo(O as obj|turf in oview(1)) //If they right click to corrode, an error will flash if its an invalid target./N
	...
	if(src.stat == DEAD)
		to_chat(src, "<span class='alium'>You can't use this while dead!</span>")
	else
		if(!(O in oview(1)))
