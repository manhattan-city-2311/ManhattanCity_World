/mob/living/carbon/human/examine(mob/user)
	...
	if(w_uniform && !(skip_gear & EXAMINE_SKIPJUMPSUIT) && w_uniform.show_examine)
		...
		if(w_uniform.blood_DNA)
			msg += "<span class='warning'>[T.He] [T.is] wearing [icon2html(w_uniform, user)] [w_uniform.gender==PLURAL?"some":"a"] [(w_uniform.blood_color != SYNTH_BLOOD_COLOUR) ? "blood" : "oil"]-stained [w_uniform.name]![tie_msg]</span><br>"
		else
			msg += "[T.He] [T.is] wearing [icon2html(w_uniform, user)] \a [w_uniform][tie_msg].<br>"

	//head
	if(head && !(skip_gear & EXAMINE_SKIPHELMET) && head.show_examine)
		if(head.blood_DNA)
			msg += "<span class='warning'>[T.He] [T.is] wearing [icon2html(head, user)] [head.gender==PLURAL?"some":"a"] [(head.blood_color != SYNTH_BLOOD_COLOUR) ? "blood" : "oil"]-stained [head.name] on [T.his] head!</span><br>"
		else
			msg += "[T.He] [T.is] wearing [icon2html(head, user)] \a [head] on [T.his] head.<br>"
	...
	if(wear_suit)
		...
		if(wear_suit.blood_DNA)
			msg += "<span class='warning'>[T.He] [T.is] wearing [icon2html(wear_suit, user)] [wear_suit.gender==PLURAL?"some":"a"] [(wear_suit.blood_color != SYNTH_BLOOD_COLOUR) ? "blood" : "oil"]-stained [wear_suit.name][tie_msg]!</span><br>"
		else
			msg += "[T.He] [T.is] wearing [icon2html(wear_suit, user)] \a [wear_suit].[tie_msg]<br>"

		//suit/armour storage
		if(s_store && !(skip_gear & EXAMINE_SKIPSUITSTORAGE) && s_store.show_examine)
			if(s_store.blood_DNA)
				msg += "<span class='warning'>[T.He] [T.is] carrying [icon2html(s_store, user)] [s_store.gender==PLURAL?"some":"a"] [(s_store.blood_color != SYNTH_BLOOD_COLOUR) ? "blood" : "oil"]-stained [s_store.name] on [T.his] [wear_suit.name]!</span><br>"
			else
				msg += "[T.He] [T.is] carrying [icon2html(s_store, user)] \a [s_store] on [T.his] [wear_suit.name].<br>"

	//back
	if(back && !(skip_gear & EXAMINE_SKIPBACKPACK) && back.show_examine)
		if(back.blood_DNA)
			msg += "<span class='warning'>[T.He] [T.has] [icon2html(back, user)] [back.gender==PLURAL?"some":"a"] [(back.blood_color != SYNTH_BLOOD_COLOUR) ? "blood" : "oil"]-stained [back] on [T.his] back.</span><br>"
		else
			msg += "[T.He] [T.has] [icon2html(back, user)] \a [back] on [T.his] back.<br>"

	//left hand
	if(l_hand && l_hand.show_examine)
		if(l_hand.blood_DNA)
			msg += "<span class='warning'>[T.He] [T.is] holding [icon2html(l_hand, user)] [l_hand.gender==PLURAL?"some":"a"] [(l_hand.blood_color != SYNTH_BLOOD_COLOUR) ? "blood" : "oil"]-stained [l_hand.name] in [T.his] left hand!</span><br>"
		else
			msg += "[T.He] [T.is] holding [icon2html(l_hand, user)] \a [l_hand] in [T.his] left hand.<br>"

	//right hand
	if(r_hand && r_hand.show_examine)
		if(r_hand.blood_DNA)
			msg += "<span class='warning'>[T.He] [T.is] holding [icon2html(r_hand, user)] [r_hand.gender==PLURAL?"some":"a"] [(r_hand.blood_color != SYNTH_BLOOD_COLOUR) ? "blood" : "oil"]-stained [r_hand.name] in [T.his] right hand!</span><br>"
		else
			msg += "[T.He] [T.is] holding [icon2html(r_hand, user)] \a [r_hand] in [T.his] right hand.<br>"

	//gloves
	if(gloves && !(skip_gear & EXAMINE_SKIPGLOVES) && gloves.show_examine)
		if(gloves.blood_DNA)
			msg += "<span class='warning'>[T.He] [T.has] [icon2html(gloves, user)] [gloves.gender==PLURAL?"some":"a"] [(gloves.blood_color != SYNTH_BLOOD_COLOUR) ? "blood" : "oil"]-stained [gloves.name] on [T.his] hands!</span><br>"
		else
			msg += "[T.He] [T.has] [icon2html(gloves, user)] \a [gloves] on [T.his] hands.<br>"
	...
	if(belt && !(skip_gear & EXAMINE_SKIPBELT) && belt.show_examine)
		if(belt.blood_DNA)
			msg += "<span class='warning'>[T.He] [T.has] [icon2html(belt, user)] [belt.gender==PLURAL?"some":"a"] [(belt.blood_color != SYNTH_BLOOD_COLOUR) ? "blood" : "oil"]-stained [belt.name] about [T.his] waist!</span><br>"
		else
			msg += "[T.He] [T.has] [icon2html(belt, user)] \a [belt] about [T.his] waist.<br>"

	//shoes
	if(shoes && !(skip_gear & EXAMINE_SKIPSHOES) && shoes.show_examine)
		if(shoes.blood_DNA)
			msg += "<span class='warning'>[T.He] [T.is] wearing [icon2html(shoes, user)] [shoes.gender==PLURAL?"some":"a"] [(shoes.blood_color != SYNTH_BLOOD_COLOUR) ? "blood" : "oil"]-stained [shoes.name] on [T.his] feet!</span><br>"
		else
			msg += "[T.He] [T.is] wearing [icon2html(shoes, user)] \a [shoes] on [T.his] feet.<br>"
	...
	if(wear_mask && !(skip_gear & EXAMINE_SKIPMASK) && wear_mask.show_examine)
		...
		if(wear_mask.blood_DNA)
			msg += "<span class='warning'>[T.He] [T.has] [icon2html(wear_mask, user)] [wear_mask.gender==PLURAL?"some":"a"] [(wear_mask.blood_color != SYNTH_BLOOD_COLOUR) ? "blood" : "oil"]-stained [wear_mask.name] [descriptor]!</span><br>"
		else
			msg += "[T.He] [T.has] [icon2html(wear_mask, user)] \a [wear_mask] [descriptor].<br>"

	//eyes
	if(glasses && !(skip_gear & EXAMINE_SKIPEYEWEAR) && glasses.show_examine)
		if(glasses.blood_DNA)
			msg += "<span class='warning'>[T.He] [T.has] [icon2html(glasses, user)] [glasses.gender==PLURAL?"some":"a"] [(glasses.blood_color != SYNTH_BLOOD_COLOUR) ? "blood" : "oil"]-stained [glasses] covering [T.his] eyes!</span><br>"
		else
			msg += "[T.He] [T.has] [icon2html(glasses, user)] \a [glasses] covering [T.his] eyes.<br>"

	//left ear
	if(l_ear && !(skip_gear & EXAMINE_SKIPEARS) && l_ear.show_examine)
		msg += "[T.He] [T.has] [icon2html(l_ear, user)] \a [l_ear] on [T.his] left ear.<br>"

	//right ear
	if(r_ear && !(skip_gear & EXAMINE_SKIPEARS) && r_ear.show_examine)
		msg += "[T.He] [T.has] [icon2html(r_ear, user)] \a [r_ear] on [T.his] right ear.<br>"
	...
	if(wear_id && wear_id.show_examine)
		...
		if(id && (id != real_name) && (get_dist(src, usr) <= 1) && prob(10))
			msg += "<span class='warning'>[T.He] [T.is] wearing [icon2html(wear_id, user)] \a [wear_id] yet something doesn't seem right...</span><br>"
		else*/
		msg += "[T.He] [T.is] wearing [icon2html(wear_id, user)] \a [wear_id].<br>"
	...
	if(intubated)
		msg += "<span class='notice'>[T.he] [T.is] intubated.</span>\n"
