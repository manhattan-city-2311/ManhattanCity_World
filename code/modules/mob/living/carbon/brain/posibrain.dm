...
/obj/item/device/mmi/digital/posibrain/proc/transfer_personality(var/mob/candidate)
	...
	to_chat(src.brainmob, "<b>Remember, the purpose of your existence is to serve the citizens and the city. Above all else, do no harm.</b>")
	to_chat(src.brainmob, "<b>Use say #b to speak to other artificial intelligences.</b>")
	src.brainmob.mind.assigned_role = "Positronic Brain"

	var/turf/T = get_turf_or_move(src.loc)
	for (var/mob/M in viewers(T))
		M.show_message(SPAN_INFO("The positronic brain beeps as it loads a personality."))
...
/obj/item/device/mmi/digital/posibrain/proc/reset_search() //We give the players sixty seconds to decide, then reset the timer.
	...
	var/turf/T = get_turf_or_move(src.loc)
	for (var/mob/M in viewers(T))
		M.show_message(SPAN_INFO("The positronic brain buzzes and beeps, and the golden lights fade away. Perhaps you could try again?"))
