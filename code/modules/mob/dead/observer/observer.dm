...
/mob/observer/dead/verb/analyze_air()
	...
	if(total_moles)
		...
		to_chat(src, SPAN_INFO("Temperature: [round(environment.temperature-T0C,0.1)]&deg;C ([round(environment.temperature,0.1)]K)"))
...
/mob/observer/dead/verb/bloody_doodle()
	...
	if(!check_rights(R_ADMIN, 0)) //Let's allow for admins to write in blood for events and the such.
