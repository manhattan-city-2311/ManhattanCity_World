...
/obj/item/device/microphone/verb/toggle_microphone()
	...
	src.on = !(src.on)
	to_chat(usr, "You switch [src] [src.on ? "on" : "off"].")
	if (src.on && prob(50))
...
/obj/item/device/microphone/attack_self(mob/M as mob, msg)
	...
	if (usr.client)
		if(usr.client.prefs.muted & MUTE_IC)
			...
			return
	if(!ishuman(usr))
		to_chat(usr, "<span class='warning'>You don't know how to use this!</span>")
		return
	...
	for (var/obj/loudspeaker/S in range(7, T))
		speakers ++
	if (!speakers)
		to_chat(usr, "<span class='warning'>You realise that there's no loudspeaker nearby for this to project to.</span>")
		return
...
/obj/item/device/mic_stand/attackby(obj/item/W as obj, mob/user as mob)
	if (istype(W, /obj/item/device/microphone))
		if (myMic)
			to_chat(user, "<span class='warning'>There's already a microphone on [src]!</span>")
			return
...
/obj/item/device/mic_stand/attack_hand(mob/living/user)
	if (!myMic)
		to_chat(user, "<span class='warning'>There's no microphone on [src]!</span>")
		return ..()
