...
#define MINIMUM_DEFIB_POWER 240
#define MAXIMUM_DEFIB_POWER 360

//backpack item
/obj/item/weapon/defibrillator
	name = "auto-resuscitator"
	desc = "A device that delivers powerful shocks via detachable paddles."
	icon = 'icons/obj/defibrillator.dmi'
	icon_state = "defibunit"
	item_state = "defibunit"
	var/shock_power = 280 //joules
...
/obj/item/weapon/shockpaddles/update_icon()
	...
	item_state = "defibpaddles1"
	if(cooldown)
		icon_state = "defibpaddles1_cooldown"
...
/obj/item/weapon/shockpaddles/proc/check_contact(mob/living/carbon/human/H)
	if(!combat)
		for(var/obj/item/clothing/cloth in list(H.wear_suit, H.w_uniform))
			if((cloth.body_parts_covered & UPPER_TORSO))
				return FALSE
	return TRUE
...
/obj/item/weapon/shockpaddles/proc/do_revive(mob/living/carbon/human/H, mob/user)
	...
	H.visible_message("<span class='warning'>\The [H]'s body convulses violently!</span>")
