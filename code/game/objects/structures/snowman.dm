...
/obj/structure/snowman/attackby(obj/item/W as obj, mob/user as mob)
	if(istype(W, /obj/item/clothing/head/that))
		if(tophat == 0)
			...
			tophat = 1
			to_chat(usr, "<span class = 'notice'>You put the tophat on the [name]. It looks quite dapper now.</span>")
			user.drop_item(W)
			qdel(W)
		else
			to_chat(usr, "<span class = 'notice'>[name] already has a tophat!</span>")
