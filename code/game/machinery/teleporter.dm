...
/proc/do_teleport(atom/movable/M as mob|obj, atom/destination, precision)
	...
	if(istype(M, /mob/living))
		...
		if(MM.check_contents_for(/obj/item/weapon/disk/nuclear))
			Mto_chat(M, "<span class='warning'>Something you are carrying seems to be unable to pass through the portal. Better drop it if you want to go through.</span>")
			return
	var/disky = 0
...
//Bags of Holding cause bluespace teleportation to go funky. --NeoFite
	if(istype(M, /mob/living))
		...
		if(MM.check_contents_for(/obj/item/weapon/storage/backpack/holding))
			Mto_chat(M, "<span class='warning'>The Bluespace interface on your Bag of Holding interferes with the teleport!</span>")
