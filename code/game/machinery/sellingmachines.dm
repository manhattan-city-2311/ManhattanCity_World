...
/obj/machinery/selling_machine/attackby(obj/item/I as obj, mob/user as mob)
	if(stored_item)
		src.visible_message("[icon2html(src, user)] <span class='warning'>Error: [src] already has an item stored, please sell or eject this item before continuing.</span>")
		return
	// First check if item has a valid price
	var/price = I.get_item_cost()
	if(isnull(price))
		playsound(src, 'sound/machines/buzz-sigh.ogg', 25)
		src.visible_message("[icon2html(src, user)] <span class='warning'>Ineligible item. Only commercially accepted items can be sold at this vendor.</span>")
		return

	stored_item = I
	user.drop_from_inventory(I, src)
	I.forceMove(src)
	src.visible_message("[icon2html(src, user)] [user] places [I] into [src].")
	interact(user)
...
/obj/machinery/selling_machine/proc/sell_item(mob/user as mob)
	...
	src.visible_message("[icon2html(src, user)] <b>[src]</b> chimes, \"<span class='notice'>Transaction complete! [stored_item] sold for [item_cost]CR.</span>\"")
