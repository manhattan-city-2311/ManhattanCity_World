	...
	if(willing)
		...
		if(ishuman(M))
			...
			if(H.save_mob_to_prefs()) // saves character if round is canon.
				spawn(20)
				to_chat(M, "<span class='notice'><b>Your character has now been saved.</b> All changes from this round will apply to your current character.</span>")
			else
				to_chat(M, "<span class='notice'><b>As this is not a canon round, your character will not be saved this time.</b></span>")
