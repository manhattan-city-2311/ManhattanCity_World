...
/datum/controller/gameticker/proc/pregame()
	...
	'sound/music/ravenheart1.ogg',*/
	'sound/music/nightexpess.ogg')

	send2mainirc("Server lobby is loaded and open at byond://[config.serverurl ? config.serverurl : (config.server ? config.server : "[world.address]:[world.port]")]")

	do
		pregame_timeleft = 180
		to_chat(world, SPAN_INFO("<B>Welcome to the pregame lobby!</B>"))
...
/datum/controller/gameticker/proc/setup()
	...
	if(!src.mode.can_start())
		to_world("<B>Unable to start [mode.name].</B> Not enough players readied, [mode.required_players] players needed. Reverting to pregame lobby.")
		current_state = GAME_STATE_PREGAME
		Master.SetRunLevel(RUNLEVEL_LOBBY)
		mode.fail_setup()
		mode = null
	...
	spawn(0)//Forking here so we dont have to wait for this to finish
		...
		sound_to(world, sound(null, channel = 1)) // Elar
...
/datum/controller/gameticker/proc/declare_completion()
	...
	if(!config.canonicity) //if we're not canon in config or by gamemode, nothing will save.
		to_world("<H2>This round was not canon. It was all a dream.</H2>")
		roll_titles()
	else
			...
			if(Player.stat != DEAD)
				...
				if(emergency_shuttle.departed && emergency_shuttle.evac)
					if(isNotAdminLevel(playerTurf.z))
						to_chat(Player, SPAN_INFO("<b>You survived the round, but remained on [station_name()] as [Player.real_name].</b>"))
					else
						to_chat(Player, "<font color='green'><b>You managed to survive the events in [station_name()] as [Player.real_name].</b></font>")
				else if(isAdminLevel(playerTurf.z))
					to_chat(Player, "<font color='green'><b>You successfully underwent crew transfer after events in [station_name()] as [Player.real_name].</b></font>")
				else if(issilicon(Player))
					to_chat(Player, "<font color='green'><b>You remain operational after the events in [station_name()] as [Player.real_name].</b></font>")
				else
					to_chat(Player, SPAN_INFO("<b>You missed the crew transfer after the events in [station_name()] as [Player.real_name].</b>"))
			else
				if(istype(Player,/mob/observer/dead))
					var/mob/observer/dead/O = Player
					if(!O.started_as_observer)
						to_chat(Player, "<font color='red'><b>You did not survive the events in [station_name()]...</b></font>")
				else
					to_chat(Player, "<font color='red'><b>You did not survive the events in [station_name()]...</b></font>")
	to_world("<br>")
	...
	for (var/mob/living/silicon/robot/robo in mob_list)
		...
		if (!robo.connected_ai)
			if (robo.stat != 2)
				to_world("<b>[robo.name] (Played by: [robo.key]) survived as an AI-less city bound synthetic! Its laws were:</b>")
			else
				to_world("<b>[robo.name] (Played by: [robo.key]) was unable to survive the rigors of being a city bound synthetic without an AI. Its laws were:</b>")
