...
/datum/emergency_shuttle_controller/proc/shuttle_arrived()
	if (!shuttle.location)	//at station
		if (autopilot)
			...
			if (evac)
				emergency_shuttle_docked.Announce(replacetext_char(replacetext_char(using_map.emergency_shuttle_docked_message, "%dock_name%", "[using_map.dock_name]"),  "%ETD%", "[estimated_time] minute\s"))
			else
				priority_announcement.Announce(replacetext_char(replacetext_char(using_map.shuttle_docked_message, "%dock_name%", "[using_map.dock_name]"),  "%ETD%", "[estimated_time] minute\s"))
...
/datum/emergency_shuttle_controller/proc/call_evac()
	...
	evac = 1
	emergency_shuttle_called.Announce(replacetext_char(using_map.emergency_shuttle_called_message, "%ETA%", "[estimated_time] minute\s"))
...
/datum/emergency_shuttle_controller/proc/call_transfer()
	...
	priority_announcement.Announce(replacetext_char(replacetext_char(using_map.shuttle_called_message, "%dock_name%", "[using_map.dock_name]"),  "%ETA%", "[estimated_time] minute\s"))
	atc.shift_ending()
