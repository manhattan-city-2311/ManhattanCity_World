	...
	for(var/level = 1 to 3)
		for(var/command_position in command_positions)
			...
			for(var/mob/V in candidates)
				...
				if(job.minimum_character_age <= age && age <= (job.minimum_character_age + 10))
					weightedCandidates[V] = 3 // Still a bit young.
				if((job.minimum_character_age+10) <= age && age <= (job.ideal_character_age-10))
					weightedCandidates[V] = 6 // Better.
				if((job.ideal_character_age-10) <= age && age <= (job.ideal_character_age+10))
					weightedCandidates[V] = 10 // Great.
				if((job.ideal_character_age+10) <= age && age <= (job.ideal_character_age+20))
					weightedCandidates[V] = 6 // Still good.
				// if((job.ideal_character_age+20) <= age && age <= POSITIVE_INFINITY)
				if((job.ideal_character_age+20) <= age)
