...
proc/skintone2racedescription(tone)
	switch (tone)
		if(30 to POSITIVE_INFINITY)		return "albino"
		...
		if(NEGATIVE_INFINITY to -65)	return "black"
		else					return "unknown"
...
proc/age2agedescription(age)
	switch(age)
		...
		if(70 to POSITIVE_INFINITY)	return "elderly"
		else				return "unknown"
...
/proc/do_after(mob/user, delay, atom/target = null, needhand = 1, progress = 1, incapacitation_flags = INCAPACITATION_DEFAULT, progbar_on_user = 0)
	if(!user)
		return 0
	if(!delay)
		return 1 //Okay. Done.
